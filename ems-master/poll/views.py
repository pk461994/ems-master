...
class PollViewSet(viewsets.ModelViewSet):
    ...
    lookup_field = 'id'     # We need id instead of pk

    @action(detail=True,methods=['GET'])
    def choices(self, request, id):
        question = self.get_object()
        choices = Choice.objects.filter(question = question) # LHS is the column name or parameter, RHS is value to column
...
class PollListView(generics.GenericAPIView, 
    ...
    def get(self, request, id=None):
        if id:
            return self.retrieve(request, id)   # By default it accepts a PK so we added lookup_field
        else:
...
class PollDetailView(APIView):
    def get_object(self, id):
        try:
            ...
            return Response({'errors':'Given Question Object Not Found'}, status = 400)
